...
### 2026-05-30 / v7.0 WIP — Web UI 接入（已暂停，待续）

- **版本**: `7.0`
- **目标**:
  - 将“给阿嬷的情书”前端接入 catnip-agent
  - 保留旧信匣、信纸叠层、木棉花与纸飞机素材
  - 新增最小 Web UI 运行入口，不破坏 11 层边界
- **当前进展**:
  - 前端已整理到 `webui/`
  - 新增 `src/shared/webui_server.py`
  - `src/main.py` 支持 `--webui`
  - `webui/app.js` 已支持通过 `/api/chat` 请求真实 agent
  - 已补 `tests/test_webui_server.py`
- **暂停点**:
  - 低高度窗口下，底部木棉花/纸飞机仍需继续压布局
  - 仍需继续做真实联调与流畅性验证
  - 当前目录缺少 `.git/` 与 `.venv/`，未满足施工文档的完整开工/收尾约束
- **修改文件**:
  - `src/main.py`
  - `src/shared/webui_server.py`
  - `webui/index.html`
  - `webui/styles.css`
  - `webui/app.js`
  - `webui/assets/*`
...
- **验证**:
  - 页面预览已跑通
  - 代码已做基础语法检查
  - 完整 `mypy src/` / `pytest` 尚未执行（当前目录缺少 `.venv/`）
- **Git 说明**:
  - 目标支线名：`给阿嬷的agent`
  - 当前目录不是 git 工作树，尚无法 commit / push 到 GitHub
